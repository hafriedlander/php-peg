...
class Builder
{
  ...
  public function replace($replacements, &$array = null)
  {
      ...
      if (is_array($array[$i])) {
        ...
        if (0 === count($array[$i][1])) {
          ...
          $delete = $delete
            || (preg_match('/^\s*if\s*\(/i', $array[$i][0]) && !$nextelse);
        ...
        /**
         * Handle replacing lines with null to remove,
         * or string, array of strings or Builder to replace
         **/
        if (array_key_exists($array[$i], $replacements)) {
          ...
          throw new \Exception('Unknown type passed to Builder#replace');
  ...
  {
		if (null === $array) $array = $this->lines;
    $out = array();
